...
import { Edit, Trash, Smile } from "./Icons.jsx";

const QUICK_EMOJIS = ["👍", "❤️", "😂", "🎉", "😮", "😢"];

export default function Message({ message, isOwn, canModerate, onEdit, onDelete, onReact, grouped }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        {!grouped && (
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[11px] text-gray-500">{format(new Date(message.createdAt), "d MMM HH:mm")}</span>
          </div>
        )}
        {editing ? (
          <div className="flex flex-col gap-1.5 mt-0.5">
            ...
            <span className="text-xs text-gray-400">
              escape — <button className="text-accent hover:underline" onClick={() => setEditing(false)}>iptal</button> • enter — <button className="text-accent hover:underline" onClick={submitEdit}>kaydet</button>
            </span>
          </div>
        ) : (
          ...
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {message.reactions.map((r) => (
              <button
                ...
                className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-base-750 border border-base-600 hover:border-accent text-xs text-gray-200"
              >
                <span>{r.emoji}</span>
      ...
      {!grouped ? null : (
        <div className="absolute left-4 top-0.5 text-[10px] text-gray-500 opacity-0 group-hover:opacity-100 w-10 text-center">
